...
setGeneric("get_hrt_params", def=function(thisObject) {standardGeneric("get_hrt_params")})
setMethod("get_hrt_params", "PVC", function(thisObject) {
  ...
  # Calculate TS
  slopes <- wapply(post_rrs, 5, by=1, FUN=function(y) return(lm(y ~ seq(1,5))$coefficients[2]))
  ...
  intercept <- model$coefficients[1]-(slope*(3+index)) # 3 = #pre_rrs + #irregular rrs - 1
...
setGeneric("get_rrs", def=function(thisObject) {standardGeneric("get_rrs")})
setMethod("get_rrs", "PVC", function(thisObject) {
  return(c(thisObject@pre_rrs, thisObject@coupl_rr, thisObject@compen_rr, thisObject@post_rrs))
})
